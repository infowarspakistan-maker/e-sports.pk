...
async function fix() {
  ...
  for (const docSnap of slidersSnap.docs) {
    ...
    if (data.slides && Array.isArray(data.slides)) {
      ...
      if (updated) {
        console.log("Found slider to fix:", docSnap.id);
        // I will just print the code to manually fix it in Firebase Console if this fails
      }
    }
  }
  ...
  for (const docSnap of gamesSnap.docs) {
    ...
    if (data.name && data.name.includes("Tekkem 8")) {
      console.log("Found game to fix:", docSnap.id);
    }
  }
  console.log("Done");
  process.exit(0);
